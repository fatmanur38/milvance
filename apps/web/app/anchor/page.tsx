...
export default function AnchorPage(): React.ReactElement {
  ...
  const address = state.phase === 'connected' ? state.address : null;
  const trustline = state.phase === 'connected' ? state.trustline : null;
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-8 px-6 py-12">
      ...
      {!address && (
        <p className="rounded-lg border border-current/15 p-4 text-sm">
          Connect your wallet on the{' '}
          <a className="underline" href="/wallet">
            wallet page
          </a>{' '}
          first. You sign every action yourself.
        </p>
      )}
